...
import { deserialize, getDynamicallyAddedParameterProps } from './helper';
...
interface FloatingActionMenuOutputsProps {
...
export const FloatingActionMenuOutputs = (props: FloatingActionMenuOutputsProps): JSX.Element => {
  ...
  const { schema, tokenMapPerOutputValue } = deserialize(props.initialValue);
  const dynamicParameterProps: DynamicallyAddedParameterProps[] = getDynamicallyAddedParameterProps(schema, tokenMapPerOutputValue);
  ...
  const onDynamicallyAddedParameterDelete = (schemaKey: string) => {
    ...
    if (onChange) {
      ...
      delete schema.outputValueMap[schemaKey];
      delete tokenMapPerOutputValue[schemaKey];

      // TODO(WIFUN): see if this will cause rerender (else try putting schema/tokenMapPerOutputvalue in useState approach)
      onChange({ value: serialize(schema, tokenMapPerOutputValue) });
    }
  };
